...
const PreviewView: React.FC<{ examId: string; onBack: () => void; }> = ({ examId, onBack }) => {
    ...
    // Effect to auto-zoom on small screens
    useEffect(() => {
        if (!settings || !mainContainerRef.current) return;
        ...
        const calculateZoom = () => {
            ...
            // Accurately calculate available width by subtracting padding
    ...
    const examHtml = useMemo(() => {
        ...
        return generateHtmlContent(exam, settings, 'exam', false); // Do not include print button in iframe
    }, [exam, settings]);

    const answerKeyHtml = useMemo(() => {
        if (!exam || !settings) return '';
        return generateHtmlContent(exam, settings, 'answer_key', false); // Do not include print button in iframe
    }, [exam, settings]);
    ...
    const handleExportHtml = useCallback(() => {
        ...
        const subjectPart = sanitize(exam.subject);
        const classPart = sanitize(exam.class);
        const titlePart = sanitize(exam.title);

        // Build filename from parts, filtering out any empty ones
        ...
        const fileName = `${baseName}${showAnswerKey ? '_kunci_jawaban' : ''}.html`;
        a.download = fileName;
        ...
        <div className="fixed inset-0 bg-slate-200 dark:bg-slate-950 z-50 flex flex-col print:bg-white">
            <header className="bg-[var(--bg-secondary)] shadow-md flex-shrink-0 print:hidden border-b border-[var(--border-primary)]">
                <div className="container mx-auto px-2 sm:px-4 py-2 flex items-center justify-between gap-2">
                    ...
                    {/* Center: Zoom Controls & View Toggle */}
                    <div className="flex-shrink-0 flex items-center gap-2 sm:gap-4">
                        ...
                        <div className="hidden sm:flex items-center rounded-lg bg-[var(--bg-muted)] p-0.5">
                            <button
                                ...
                                className={`px-3 py-1 text-sm font-semibold rounded-md transition-colors ${!showAnswerKey ? 'bg-[var(--bg-secondary)] text-blue-600 dark:text-slate-100 shadow-sm' : 'text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]'}`}
                            >
                                Soal
                            </button>
                            <button
                    ...
                    <div className="flex-1 flex justify-end">
                        ...
                        <div className="hidden md:flex items-center space-x-3">
                            <button onClick={handleExportHtml} className="text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] font-semibold py-2 px-4 rounded-lg flex items-center space-x-2">
                                <DownloadIcon /><span>Ekspor HTML</span>
                            </button>
                            <button onClick={handlePrint} className="bg-[var(--bg-accent)] hover:bg-[var(--bg-accent-hover)] text-[var(--text-on-accent)] font-semibold py-2 px-4 rounded-lg flex items-center space-x-2">
                                <PrinterIcon /><span>Cetak / Simpan PDF</span>
                            </button>
                        </div>
                        {/* Mobile Dropdown */}
                        <div className="md:hidden relative" ref={actionsMenuRef}>
                            <button 
                                ...
                                className="flex items-center space-x-2 bg-[var(--bg-muted)] hover:bg-[var(--bg-hover)] text-[var(--text-primary)] font-semibold py-2 px-4 rounded-lg"
                                aria-haspopup="true" aria-expanded={isActionsMenuOpen}
                            >
                                <span>Opsi</span>
                            ...
                            {isActionsMenuOpen && (
                                <div className="absolute right-0 mt-2 w-56 bg-[var(--bg-secondary)] rounded-md shadow-lg z-20 border border-[var(--border-primary)] py-1">
                                    ...
                                    <button onClick={() => { setShowAnswerKey(false); setActionsMenuOpen(false); }} className="w-full text-left flex items-center space-x-2 block px-4 py-2 text-sm text-[var(--text-primary)] hover:bg-[var(--bg-hover)]">
                                        <span>Lihat Soal</span>
                                    </button>
                                     <button onClick={() => { setShowAnswerKey(true); setActionsMenuOpen(false); }} className="w-full text-left flex items-center space-x-2 block px-4 py-2 text-sm text-[var(--text-primary)] hover:bg-[var(--bg-hover)]">
                                        <span>Lihat Kunci Jawaban</span>
                                    </button>
                                    <div className="my-1 border-t border-[var(--border-primary)]"></div>
                                    <button onClick={() => { handleExportHtml(); setActionsMenuOpen(false); }} className="w-full text-left flex items-center space-x-2 block px-4 py-2 text-sm text-[var(--text-primary)] hover:bg-[var(--bg-hover)]">
                                        <DownloadIcon /><span>Ekspor HTML</span>
                                    </button>
                                    <button onClick={() => { handlePrint(); setActionsMenuOpen(false); }} className="w-full text-left flex items-center space-x-2 block px-4 py-2 text-sm text-[var(--text-primary)] hover:bg-[var(--bg-hover)]">
                                        <PrinterIcon /><span>Cetak / Simpan PDF</span>
                                    </button>
                                </div>
                            )}
            ...
            <main ref={mainContainerRef} className="flex-grow overflow-auto p-4 sm:p-8 flex justify-center" style={{ scrollbarWidth: 'thin', scrollbarColor: '#94a3b8 #e2e8f0' }}>
                 <div 
                    ...
                    style={{ 
                        ...
                        width: settings.paperSize === 'A4' ? '210mm' : settings.paperSize === 'F4' ? '215mm' : '216mm' // Set width for transform origin
                    }}
                >
                    <iframe
                        ref={iframeRef}
