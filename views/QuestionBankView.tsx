...
const QuestionPreview: React.FC<{ question: Question }> = ({ question }) => {
    return <div className="prose prose-sm dark:prose-invert max-w-none text-[var(--text-primary)]" dangerouslySetInnerHTML={{ __html: sanitizeRichHtml(question.text) }} />;
};
...
const QuestionBankView: React.FC<QuestionBankViewProps> = ({ isModalMode = false, onAddQuestions, onClose }) => {
    ...
    return (
        <div className={`flex flex-col h-full ${containerPadding}`}>
            ...
            <div className="app-surface p-4 rounded-[var(--radius-card)] mb-5">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <input type="text" placeholder="Cari teks soal..." aria-label="Cari teks soal" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="p-3 border border-[var(--border-secondary)] rounded-[var(--radius-control)] bg-[var(--bg-secondary)]" />
                <select aria-label="Filter berdasarkan mata pelajaran" value={subjectFilter} onChange={e => setSubjectFilter(e.target.value)} className="p-3 border border-[var(--border-secondary)] rounded-[var(--radius-control)] bg-[var(--bg-secondary)]">
                    <option value="">Semua Mata Pelajaran</option>
                    {uniqueSubjects.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                <select aria-label="Filter berdasarkan kelas" value={classFilter} onChange={e => setClassFilter(e.target.value)} className="p-3 border border-[var(--border-secondary)] rounded-[var(--radius-control)] bg-[var(--bg-secondary)]">
            ...
            {isModalMode && (
                <div className="flex-shrink-0 pt-4 border-t border-[var(--border-primary)] flex justify-end items-center space-x-3">
                    ...
                    <button onClick={onClose} className="bg-[var(--bg-muted)] hover:bg-[var(--bg-hover)] text-[var(--text-primary)] font-semibold py-2 px-4 rounded-[var(--radius-control)]">Batal</button>
                    <button onClick={handleAddSelected} disabled={selectedQuestionIds.size === 0} className="bg-[var(--bg-accent)] hover:bg-[var(--bg-accent-hover)] text-[var(--text-on-accent)] font-semibold py-2 px-4 rounded-[var(--radius-control)] flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed">
                       <PlusIcon/>
