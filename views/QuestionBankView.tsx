...
const QuestionPreview: React.FC<{ question: Question }> = ({ question }) => {
    // A simplified renderer for question text to avoid complex layouts
...
const QuestionBankView: React.FC<QuestionBankViewProps> = ({ isModalMode = false, onAddQuestions, onClose }) => {
    ...
    const handleDelete = (bankId: string) => {
        showConfirm({
            ...
            confirmLabel: 'Hapus',
            onConfirm: () => {
                setBank(deleteQuestionFromBank(bankId));
    ...
    const handleAddSelected = () => {
        const selectedQuestions = bank
            .filter(bq => selectedQuestionIds.has(bq.question.id))
            .map(bq => bq.question);
        ...
        <div className={`flex flex-col h-full ${containerPadding}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <input
                    ...
                    className="p-2 border border-[var(--border-secondary)] rounded-md bg-[var(--bg-secondary)]"
                />
                        ...
                        <div key={bq.bankId} className="bg-[var(--bg-secondary)] p-4 rounded-lg shadow-md border border-[var(--border-primary)] flex gap-4">
                            {isModalMode && (
                                <div className="flex-shrink-0 flex items-center justify-center">
                                    <input 
                                        type="checkbox" 
                                        aria-label={`Pilih soal: ${bq.question.text.substring(0, 50)}`}
                                        className="h-5 w-5 rounded text-blue-600 bg-transparent border-[var(--border-secondary)] focus:ring-blue-500"
                                        checked={selectedQuestionIds.has(bq.question.id)}
                                        onChange={() => handleSelectQuestion(bq.question.id)}
                                    />
                                </div>
                            )}
                            <div className="flex-grow">
                                ...
                                <div className="mt-3 pt-3 border-t border-[var(--border-primary)] flex justify-between items-center text-xs text-[var(--text-secondary)]">
                                    ...
                                    {!isModalMode && (
                                        <button onClick={() => handleDelete(bq.bankId)} aria-label="Hapus soal dari bank" className="p-1 text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full">
                                            <TrashIcon />
                                        </button>
                                    )}
                                </div>
            ...
            {isModalMode && (
                <div className="flex-shrink-0 pt-4 border-t border-[var(--border-primary)] flex justify-end items-center space-x-3">
                    ...
                    <button onClick={onClose} className="bg-[var(--bg-muted)] hover:bg-[var(--bg-hover)] text-[var(--text-primary)] font-semibold py-2 px-4 rounded-lg">
                        Batal
                    </button>
                    <button 
                        onClick={handleAddSelected} 
                        disabled={selectedQuestionIds.size === 0}
                        className="bg-[var(--bg-accent)] hover:bg-[var(--bg-accent-hover)] text-[var(--text-on-accent)] font-semibold py-2 px-4 rounded-lg flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                       <PlusIcon/>
